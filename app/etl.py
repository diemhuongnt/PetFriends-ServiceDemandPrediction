...
GROUP BY 
    ...
    cs.DiscountFrom, 
    cs.DiscountTo,
    -- Các CASE WHEN cũng cần viết lại y chang ở GROUP BY
    CASE 
        WHEN DATEPART(WEEKDAY, acs.DateGiven) = 1 THEN 6
        ELSE DATEPART(WEEKDAY, acs.DateGiven) - 2
    END,
...
# Sử dụng khoảng thời gian cố định để tạo bảng cartesian: từ 30 ngày trước đến hôm nay
...
merged['discount_from'] = merged['discount_from'].fillna(default_date)
merged['discount_to'] = merged['discount_to'].fillna(default_date)

# Compute discount_flag: nếu discount_from hoặc discount_to == default_date, flag = 0; else so sánh
def compute_discount_flag(row):
    # Chuyển đổi các giá trị ngày/thời gian về pd.Timestamp để đảm bảo cùng kiểu dữ liệu
    ...
    # So sánh sau khi đã ép kiểu, trả về 1 nếu date nằm trong khoảng [discount_from, discount_to], ngược lại 0
...
# Bỏ những service chưa có đủ data training (ít nhất 3 ngày)
valid_services = final_df.groupby("service_id").filter(lambda x: x['booking_count'].gt(0).sum() >= 3)
final_df = valid_services.copy()
